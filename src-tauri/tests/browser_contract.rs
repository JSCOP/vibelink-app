...
fn page_originated_navigation_invalidates_exact_persisted_annotations() {
    ...
    let input = BrowserAnnotationInput {
        ...
        selector: "form#checkout > button#submit".to_string(),
        full_path: "body > main > form#checkout > button#submit".to_string(),
        role: "button".to_string(),
        react_components: "<Checkout> <SubmitButton>".to_string(),
        html_snippet: "<button id=\"submit\" type=\"submit\">Submit</button>".to_string(),
        accessible_name: "Submit".to_string(),
        nearby_text: vec!["Cancel".to_string()],
        ancestor_path: vec![
            "form[role=form]".to_string(),
            "main".to_string(),
            "body".to_string(),
        ],
        bounds: bounds(),
