...
fn broadcast_output(
    ...
    let base_frame: Arc<[u8]> = Arc::from(
        frame_output(pane_id, pane_generation, output_sequence, data).into_boxed_slice(),
    );
    ...
    fn websocket_queue_is_byte_bounded() {
        ...
        for output_sequence in 1..=10 {
            let frame: Arc<[u8]> = Arc::from(
                frame_output(pane_id, 1, output_sequence, &data).into_boxed_slice(),
            );
