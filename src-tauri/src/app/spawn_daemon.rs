use crate::{
    ...
    protocol::{
        ...
        ReplyResult, Req, DAEMON_AUTH_REQUIRED, DAEMON_PROTOCOL_MISMATCH, DAEMON_PROTOCOL_VERSION,
    },
};
...
pub struct DaemonReplacement {
    pub from_version: Option<String>,
    pub to_version: String,
    pub terminated_panes: u32,
}
...
pub fn take_daemon_replacement() -> Option<DaemonReplacement> {
    DAEMON_REPLACEMENT
        .get()?
        .lock()
        .expect("daemon replacement mutex poisoned")
        .take()
}
...
pub fn ensure_daemon_for(client_kind: ClientKind) -> Result<DaemonStream> {
    ...
    let result = ensure_daemon_with_recovery_for(&mut recovery, client_kind);
    let _ = DAEMON_REPLACEMENT.set(Mutex::new(recovery.replacement));
    result
}
...
pub(super) fn ensure_daemon_with_recovery_for(
    ...
    let initial_error = loop {
        match connect_ready_daemon(client_kind) {
            ...
            Err(err) if attempt == 0 && err.kind == StartupAttemptErrorKind::Connect => {
                attempt += 1;
    ...
    let mut last_error = if let Some(err) = initial_error {
...
const STARTUP_PANE_COUNT_REQ: Req = STARTUP_PING_REQ + 1;

fn running_daemon_pane_count(stream: DaemonStream) -> u32 {
    let count = Arc::new(AtomicU32::new(0));
    let output = Arc::clone(&count);
    if run_daemon_step_with_timeout(
        stream,
        STARTUP_PING_TIMEOUT,
        "vibelink-daemon-pane-count",
        "daemon live pane count",
        move |stream| {
            output.store(query_running_daemon_pane_count(stream)?, Ordering::Release);
            Ok(())
        },
    )
    .is_err()
...
fn query_running_daemon_pane_count<S>(stream: &mut S) -> Result<u32>
where
    S: Read + Write,
{
    write_frame(
        ...
        &ClientToDaemon::ListSessions {
            req: STARTUP_PANE_COUNT_REQ,
        },
    )
    .context("write daemon session list request")?;
    match read_frame::<_, DaemonToClient>(stream).context("read daemon session list")? {
        DaemonToClient::Reply {
            req,
            result: ReplyResult::Sessions(sessions),
        } if req == STARTUP_PANE_COUNT_REQ => Ok(sessions.into_iter().fold(0, |total, session| {
            total.saturating_add(u32::try_from(session.pane_count).unwrap_or(u32::MAX))
        })),
        DaemonToClient::Error { message, .. } => {
            bail!("daemon rejected session list request: {message}")
        }
        other => bail!("unexpected daemon session list response: {other:?}"),
    }
}
