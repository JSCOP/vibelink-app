...
/// artifacts in place.
const HOOK_SCRIPT_VERSION: u32 = 3;
...
const AGENT_HOOK_SPECS: &[AgentHookSpec] = &[
    ...
    AgentHookSpec {
        ...
        // `turn_end` fires after every provider response/tool cycle. Only the
        // terminal `agent_end` settle represents the user's completed prompt.
        completion_events: &["agent_end"],
    },
    AgentHookSpec {
        id: "opencode",
...
mod tests {
    ...
    fn filesystem_round_trip_leaves_no_residue() {
        ...
        assert!(render_drop_in(spec("omp"), &cli_exe().expect("cli")).contains("agent_end"));
    ...
    #[test]
    fn omp_reports_only_terminal_agent_end() {
        let body = render_drop_in(spec("omp"), Path::new("C:/vl/vibelink.exe"));

        assert!(body.contains("pi.on(\"agent_end\""));
        assert!(!body.contains("pi.on(\"turn_end\""));
        assert!(body.contains("event.willContinue"));
        assert!(body.contains("stopReason === \"aborted\""));
        assert!(body.contains("stopReason === \"error\""));
    }

    #[test]
    fn specs_follow_current_agent_hook_locations_and_completion_events() {
        ...
        assert_eq!(omp.completion_events, &["agent_end"]);
        let omp_module = render_drop_in(omp, Path::new("C:/vl/vibelink.exe"));
        assert!(omp_module.contains("pi.on(\"agent_end\""));
