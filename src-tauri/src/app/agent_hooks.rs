...
//! 2. **Reversible.** `uninstall` removes our config entries, drop-in modules,
//!    legacy VibeLink entries, and generated launchers.
...
mod tests {
    ...
    fn filesystem_round_trip_leaves_no_residue() {
        ...
        assert!(
            render_drop_in(spec("omp"), &script_path("omp").expect("script")).contains("turn_end")
        );
        assert_eq!(
    ...
    fn specs_follow_current_agent_hook_locations_and_turn_events() {
        ...
        let omp_module = render_drop_in(omp, Path::new("C:/vl/hook.cmd"));
