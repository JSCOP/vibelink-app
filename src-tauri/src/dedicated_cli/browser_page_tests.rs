...
fn snapshot_refs_survive_between_commands() {
    ...
    fs::write(snapshot_state_path(&root, "TARGET/1"), b"{").expect("corrupt snapshot state");
...
fn snapshot_state_writes_are_bounded_and_pruned() {
    ...
    assert!(snapshot_state_path(&root, &format!("target-{MAX_SNAPSHOT_STATE_FILES}")).exists());
    let _ = fs::remove_dir_all(root);
}
...
fn resolved_detached_node_is_rejected_as_a_stale_ref() {
    ...
    assert!(calls
        .iter()
        .all(|(method, _)| method != "Input.dispatchMouseEvent"));
    let _ = fs::remove_dir_all(root);
}
