...
pub fn pane_order(layout_json: Option<&str>, panes: &[PaneMeta]) -> Vec<Uuid> {
    ...
    let Some(layout) = terminal_layout(raw) else {
        return fallback;
    };
    let Some(panels) = layout.get("panels").and_then(Value::as_object) else {
        return fallback;
    };
    ...
    let Some(root) = layout.pointer("/grid/root") else {
        return fallback;
    };
...
fn traverse(node: &Value, output: &mut Vec<String>) {
    match node.get("type").and_then(Value::as_str) {
        ...
        Some("leaf") => {
            if let Some(views) = node
                .get("data")
                .and_then(|data| data.get("views"))
                .and_then(Value::as_array)
            {
...
mod tests {
    ...
    fn terminal_dockview_depth_first_order_maps_panel_params() {
        ...
        assert_eq!(
            pane_order(Some(&state.to_string()), &[pane(a), pane(b), pane(c)]),
            vec![b, a, c]
        );
    }
