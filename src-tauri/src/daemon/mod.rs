...
use crate::daemon::automation::{AutomationService, AutomationWorktreeProvision, CreateAutomation};
                        ...
                        Ok(claims) => {
                            for claim in claims {
                                ...
                                let _ = thread::Builder::new()
                                    ...
                                    .spawn(move || {
                                        if let Err(error) = automation.execute_with_worktree(
                                            &claim,
                                            &workspace,
...
fn dispatch_automation_cli(
    ...
    match command.action {
        ...
        AutomationAction::Run => {
            ...
            Ok(serde_json::to_value(automation.execute_with_worktree(
                &claim,
                &workspace,
                |record, claim, workspace, planned| {
                    provision_automation_worktree(
...
fn provision_automation_worktree(
    ...
    let base_sha = exact_worktree_base_snapshot(&authority.repository_root)?;
