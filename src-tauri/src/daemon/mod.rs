        ...
        match stream {
            Ok(stream) => {
                ...
                thread::Builder::new()
                    ...
                    .spawn(move || handle_connection(stream, state, sessions_path))?;
            }
...
fn reconstruct_sessions(state: SharedState, sessions_path: &Path) -> Result<()> {
    for persisted in load_sessions(sessions_path)? {
        ...
        for cfg in persisted.panes {
            ...
            if let Err(err) = spawn_pane_for_session(Arc::clone(&state), sessions_path.to_path_buf(), persisted.id, cfg) {
...
fn handle_connection(stream: LocalSocketStream, state: SharedState, sessions_path: Arc<PathBuf>) {
    ...
    loop {
        let msg = match read_frame::<_, ClientToDaemon>(&mut reader) {
            Ok(msg) => msg,
            Err(crate::protocol::FrameError::Io(err)) if err.kind() == io::ErrorKind::UnexpectedEof => break,
            Err(err) => {
        ...
        if let Err(err) = dispatch_message(Arc::clone(&state), &sessions_path, client_id, &tx, msg) {
...
fn dispatch_message(
    ...
    match msg {
        ...
        ClientToDaemon::ListSessions { req } => {
            let sessions = state.lock().expect("daemon state mutex poisoned").list_sessions();
            send(tx, DaemonToClient::Reply { req, result: ReplyResult::Sessions(sessions) })
        }
        ClientToDaemon::CreateSession { req, name } => {
            let meta = state.lock().expect("daemon state mutex poisoned").create_session(name);
            persist_state(&state, sessions_path)?;
            send(tx, DaemonToClient::Reply { req, result: ReplyResult::SessionCreated(meta) })
        }
        ClientToDaemon::RenameSession { req, session_id, name } => {
            state.lock().expect("daemon state mutex poisoned").rename_session(session_id, name)?;
            persist_state(&state, sessions_path)?;
            send_ok(tx, req)
        }
        ClientToDaemon::DeleteSession { req, session_id } => {
            state.lock().expect("daemon state mutex poisoned").delete_session(session_id)?;
            persist_state(&state, sessions_path)?;
            send_ok(tx, req)
        }
        ClientToDaemon::AttachSession { req, session_id } => {
            let (layout_json, panes) = state.lock().expect("daemon state mutex poisoned").attach_session(session_id)?;
            send(tx, DaemonToClient::Reply { req, result: ReplyResult::Attached { layout_json, panes } })
        }
        ClientToDaemon::DetachSession { session_id } => {
            state.lock().expect("daemon state mutex poisoned").detach_session(client_id, session_id);
            Ok(())
        }
        ClientToDaemon::SaveLayout { session_id, layout_json } => {
            state.lock().expect("daemon state mutex poisoned").save_layout(session_id, layout_json)?;
            persist_state(&state, sessions_path)
        }
        ClientToDaemon::SpawnPane { req, session_id, cfg } => {
            let meta = spawn_pane_for_session(Arc::clone(&state), sessions_path.to_path_buf(), session_id, cfg)?;
            persist_state(&state, sessions_path)?;
            send(tx, DaemonToClient::Reply { req, result: ReplyResult::PaneSpawned(meta) })
        }
        ClientToDaemon::AttachPane { pane_id } => {
            info!(%client_id, %pane_id, "attaching pane");
            state.lock().expect("daemon state mutex poisoned").attach_pane(client_id, pane_id)?;
            Ok(())
        }
        ClientToDaemon::WritePane { pane_id, data } => {
            state.lock().expect("daemon state mutex poisoned").write_pane(pane_id, &data)?;
            Ok(())
        }
        ClientToDaemon::ResizePane { pane_id, cols, rows } => {
            state.lock().expect("daemon state mutex poisoned").resize_pane(pane_id, cols, rows)?;
            Ok(())
        }
        ClientToDaemon::ClosePane { req, pane_id } => {
            state.lock().expect("daemon state mutex poisoned").close_pane(pane_id)?;
            persist_state(&state, sessions_path)?;
            send_ok(tx, req)
        }
    }
...
fn request_id(msg: &ClientToDaemon) -> Option<crate::protocol::Req> {
    match msg {
        ...
        | ClientToDaemon::ClosePane { req, .. } => Some(*req),
...
fn persist_state(state: &SharedState, sessions_path: &Path) -> Result<()> {
    let persisted = state.lock().expect("daemon state mutex poisoned").persisted_sessions();
...
fn spawn_pane_for_session(
    ...
    state.lock().expect("daemon state mutex poisoned").pane_metas(session_id)?;

    let pane_id = cfg.pane_id;
    let spawned = Pane::spawn(cfg)?;
    let child = spawned.pane.child();
    let reader = spawned.reader;
    let meta = state.lock().expect("daemon state mutex poisoned").insert_pane(session_id, spawned.pane)?;
...
fn read_pane_loop(state: SharedState, pane_id: Uuid, mut reader: Box<dyn Read + Send>, child: SharedChild, sessions_path: Arc<PathBuf>) {
    ...
    loop {
        match reader.read(&mut buf) {
            ...
            Ok(n) => {
                ...
                let senders = state.lock().expect("daemon state mutex poisoned").record_output(pane_id, &bytes);
                for sender in senders {
                    let _ = sender.send(DaemonToClient::Output { pane_id, data: bytes.clone() });
                }
            }
            Err(err) => {
    ...
    let senders = state.lock().expect("daemon state mutex poisoned").mark_exited(pane_id);
    for sender in senders {
...
fn send_ok(tx: &Sender<DaemonToClient>, req: Req) -> Result<()> {
    send(tx, DaemonToClient::Reply { req, result: ReplyResult::Ok })
}
