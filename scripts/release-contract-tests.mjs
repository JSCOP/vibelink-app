...
test('candidate gate accepts only a matching tag and synchronized desktop versions', async () => {
  ...
  const candidate = { version, desktopCommit: 'a'.repeat(40) }
  const result = await validateCandidateRelease({ manifest: manifest(candidate), desktopRoot, tag: `v${version}` })
  assert.deepEqual(result.versions, versions)
  await assert.rejects(
    () => validateCandidateRelease({ manifest: manifest(candidate), desktopRoot, tag: 'v99.99.99' }),
    new RegExp(`Release tag must equal candidate version v${version.replaceAll('.', '\\.')}`),
  )
})
