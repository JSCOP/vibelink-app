...
function OrchestrationRunPanel() {
  ...
  const refreshRuns = useCallback(async () => {
    if (!sessionId) return
    const next = await orchestrationRequest<OrchestrationRun[]>('runs.list', { id: sessionId })
    setRuns(next)
    setRun((current) => next.find((candidate) => candidate.id === current?.id) ?? next[0] ?? null)
  }, [sessionId])

  const refresh = useCallback(async (runId: string) => {
    const consumerId = `desktop:${sessionId ?? 'unknown'}`
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      setEvents([])
      setError(null)
      void refreshRuns().catch((cause) => setError(cause instanceof Error ? cause.message : String(cause)))
    }, 0)
  ...
  useEffect(() => {
    ...
    const initialTimer = window.setTimeout(() => {
      void refresh(runId).catch((cause) => setError(cause instanceof Error ? cause.message : String(cause)))
    }, 0)
    const timer = window.setInterval(() => {
      void Promise.all([refresh(runId), refreshRuns()]).catch((cause) => setError(cause instanceof Error ? cause.message : String(cause)))
    }, 2_000)
    return () => {
      window.clearTimeout(initialTimer)
      window.clearInterval(timer)
    }
  }, [refresh, refreshRuns, runId])
  ...
  const createRun = (
    <div className="orchestration-empty orchestration-create-run">
      ...
      <button type="button" disabled={busy || !goal.trim()} onClick={() => void execute(async () => {
        const created = await orchestrationRequest<OrchestrationRun>('run.create', { sessionId, goal: goal.trim(), policy: { maxConcurrent: 4 } })
        setRun(created)
        await refreshRuns()
  ...
  return (
    <div className="orchestration-run-panel">
      <header className="orchestration-run-header">
        <div>
          <select value={run.id} onChange={(event) => setRun(runs.find((candidate) => candidate.id === event.target.value) ?? run)} aria-label="Durable run">
        ...
        <div className="orchestration-run-actions">
          ...
          {['planning', 'running', 'waiting'].includes(run.status) ? <button type="button" disabled={busy} onClick={() => void execute(async () => { await orchestrationRequest('schedule.ready', { runId: run.id, expectedRunRevision: run.revision }); await refresh(run.id) })}><Workflow size={14} /> Advance DAG</button> : null}
          {['planning', 'running', 'waiting', 'paused'].includes(run.status) ? <button type="button" disabled={busy} onClick={() => void execute(async () => { await orchestrationRequest('run.cancel', { runId: run.id, expectedRunRevision: run.revision }); await refresh(run.id) })}><Square size={14} /> Cancel</button> : null}
          {run.status === 'completed' ? <button type="button" disabled={busy} onClick={() => void execute(async () => { await orchestrationRequest('run.accept', { runId: run.id, expectedRunRevision: run.revision, payload: { source: 'desktop' } }); await refresh(run.id) })}><Check size={14} /> Accept</button> : null}
          {!['cancelled'].includes(run.status) ? <button type="button" disabled={busy} onClick={() => void execute(async () => { await orchestrationRequest('run.reject', { runId: run.id, expectedRunRevision: run.revision, payload: { source: 'desktop' } }); await refresh(run.id) })}><X size={14} /> Reject</button> : null}
          <button type="button" disabled={busy} onClick={() => { setRun(null); setGoal('Coordinate this workspace') }}><ListPlus size={14} /> New</button>
        </div>
      </header>
      ...
      <section className="orchestration-message-feed" aria-label="Live agents and worktrees">
        ...
        {agents.length === 0 ? <p>No bound agents yet.</p> : agents.map((agent) => {
          ...
          return <article key={agent.id} className="orchestration-agent-card">
            ...
            {dispatch?.worktree ? <><p>{dispatch.worktree.branch}</p><small title={dispatch.worktree.worktreePath}>{dispatch.worktree.worktreePath}</small><p>Base {dispatch.worktree.baseRevision.slice(0, 12)}</p></> : null}
      ...
      {pendingGates.length ? <section className="orchestration-gates" aria-label="Decision gates">{pendingGates.map((gate) => <article key={gate.id}>
        <strong>{gate.prompt}</strong><div>{(gate.options.length ? gate.options : ['approve']).map((option) => <button key={option} type="button" disabled={busy} onClick={() => void execute(async () => { await orchestrationRequest('gate.resolve', { gateId: gate.id, resolution: { decision: option }, expectedRunRevision: run.revision }); await refresh(run.id) })}><Check size={13} /> {option}</button>)}</div>
